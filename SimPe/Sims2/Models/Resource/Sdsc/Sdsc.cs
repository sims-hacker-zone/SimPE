...
using SimPe.Sims2.Views.Resource.Sdsc;

namespace SimPe.Sims2.Models.Resource.Sdsc;

public partial class Sdsc(Resource resource) : ObservableObject, IWrapper
{
	...
	[ObservableProperty] private ushort sitting;

	[ObservableProperty] private ushort moneyOverHead;

	[ObservableProperty] private SdscPersonality personality = new();

	[ObservableProperty] private ushort uniEffort;

	[ObservableProperty] private ushort currentOutfit;

	[ObservableProperty] private SdscSkills skills = new();

	[ObservableProperty] private ushort groupTalkStateFlags;

	[ObservableProperty] private ushort bodyTemp;

	[ObservableProperty] private ushort currentInteractionIndex;

	[ObservableProperty] private short malePreference;

	[ObservableProperty] private short femalePreference;

	[ObservableProperty] private ushort jobData;

	[ObservableProperty] private ushort interactionData1;

	[ObservableProperty] private ushort subQueueInteractionCount;

	[ObservableProperty] private ushort tickCounter;

	[ObservableProperty] private ushort interactionData2;

	[ObservableProperty] private ushort staticMotives;

	[ObservableProperty] private ushort censorshipFlags;

	[ObservableProperty] private ushort neighborID;

	[ObservableProperty] private ushort personType;

	[ObservableProperty] private ushort priority;

	[ObservableProperty] private ushort greetStatus;

	[ObservableProperty] private ushort visitorSchedule;

	[ObservableProperty] private ushort autonomyLevel;

	[ObservableProperty] private ushort routeSlot;

	[ObservableProperty] private ushort routeMultiSlotIndex;

	[ObservableProperty] private ushort routeStatus;

	[ObservableProperty] private ushort routeGoal;

	[ObservableProperty] private ushort lookAtObjectID;

	[ObservableProperty] private ushort lookAtSlotID;

	[ObservableProperty] private ushort lookAtState;

	[ObservableProperty] private ushort lookAtTimeRemaining;

	[ObservableProperty] private ushort nextQueuedInteractionIndex;

	[ObservableProperty] private ushort aspiration;

	[ObservableProperty] private SdscPersonality originalPersonality = new();

	[ObservableProperty] private ushort simUIIconFlags;

	[ObservableProperty] private ushort findBestActionResult;

	[ObservableProperty] private ushort memoryScore;

	[ObservableProperty] private ushort routeStartSlot;

	[ObservableProperty] private ushort schoolGrade;

	[ObservableProperty] private ushort jobPromotionLevel;

	[ObservableProperty] private ushort personAge;

	[ObservableProperty] private ushort socialMenuObjectID;

	[ObservableProperty] private ushort skinColor;

	[ObservableProperty] private ushort familyInstance;

	[ObservableProperty] private ushort routeResult;

	[ObservableProperty] private ushort jobPerformance;

	[ObservableProperty] private ushort isSwimming;

	[ObservableProperty] private EnumDisplayNameItem<SdscGender> gender;

	[ObservableProperty] private ushort @private;

	[ObservableProperty] private ushort lingeringHouseInstance;

	[ObservableProperty] private ushort ghostFlags;

	[ObservableProperty] private ushort paidTimeOff;

	[ObservableProperty] private ushort zodiacSign;

	[ObservableProperty] private ushort nonInterruptible;

	[ObservableProperty] private ushort nextQueuedInteractionContinuation;

	[ObservableProperty] private ushort footprintExtension;

	[ObservableProperty] private ushort renderDisplayFlags;

	[ObservableProperty] private ushort subQueueMasterInteractionObjectID;

	[ObservableProperty] private ushort subQueueMasterInteractionIndex;

	[ObservableProperty] private ushort nextSubQueueInteractionIndex;

	[ObservableProperty] private ushort nextSubQueueInteractionObjectID;

	[ObservableProperty] private ushort nextQueuedInteractionObjectID;

	[ObservableProperty] private ushort currentInteractionObjectID;

	[ObservableProperty] private ushort bodyFlags;

	[ObservableProperty] private ushort fatness;

	[ObservableProperty] private ushort uniGrade;

	[ObservableProperty] private ushort voiceType;
	[ObservableProperty] private uint jobGUID;

	[ObservableProperty] private ushort ageDaysLeft;

	[ObservableProperty] private ushort daysInPreviousAge;

	[ObservableProperty] private short decayHunger;

	[ObservableProperty] private short decayComfort;

	[ObservableProperty] private short decayBladder;

	[ObservableProperty] private short decayEnergy;

	[ObservableProperty] private short decayHygiene;

	[ObservableProperty] private short decaySocial;

	[ObservableProperty] private ushort decayShopping;

	[ObservableProperty] private short decayFun;

	[ObservableProperty] private ushort currentRunningInteractionIndex;

	[ObservableProperty] private ushort currentRunningInteractionObjectID;

	[ObservableProperty] private ushort motivePower;

	[ObservableProperty] private ushort workOutfitID;

	[ObservableProperty] private ushort decayScratchChew;

	[ObservableProperty] private uint schoolGUID;

	[ObservableProperty] private ushort currentInteractionGUID;

	[ObservableProperty] private ushort interactionsLinkedDeleted;

	[ObservableProperty] private ushort sunshineMotive;

	[ObservableProperty] private ushort locoWeight0;

	[ObservableProperty] private ushort locoWeight1;

	[ObservableProperty] private ushort locoPersonalityIndex;

	[ObservableProperty] private ushort locoPersonalityWeight;

	[ObservableProperty] private ushort locoMoodIndex;

	[ObservableProperty] private ushort locoMoodWeight;

	[ObservableProperty] private ushort motivesNotIncludedInMood;

	[ObservableProperty] private uint outfitSourceGUID;

	[ObservableProperty] private ushort environmentScoreOverride;

	[ObservableProperty] private ushort fitnessPreference;

	[ObservableProperty] private ushort pension;

	[ObservableProperty] private SdscInterests interests = new();

	[ObservableProperty] private ushort unselectable;

	[ObservableProperty] private ushort nPCType;

	[ObservableProperty] private ushort ageDuration;

	[ObservableProperty] private ushort subQueueInteractionObjectID;

	[ObservableProperty] private ushort selectionFlags;

	[ObservableProperty] private ushort personFlags;

	[ObservableProperty] private ushort aspirationScore;

	[ObservableProperty] private ushort aspirationRewardPointsSpent;

	[ObservableProperty] private ushort aspirationScoreRaw;

	[ObservableProperty] private ushort moodBooster;

	[ObservableProperty] private ushort currentInteractionJoinable;

	[ObservableProperty] private ushort unlinked;

	[ObservableProperty] private ushort interactionAutonomous;

	[ObservableProperty] private uint retiredJobGUID;

	[ObservableProperty] private ushort retiredJobLevel;

	[ObservableProperty] private uint collegeMajorGUID;

	[ObservableProperty] private ushort remainingTimeInSemester;

	[ObservableProperty] private ushort uniFlags;

	[ObservableProperty] private ushort semester;

	[ObservableProperty] private ushort youngAdult;

	[ObservableProperty] private ushort influenceBarLevel;

	[ObservableProperty] private ushort influenceMinimum;

	[ObservableProperty] private ushort influence;

	[ObservableProperty] private ushort routeStartSlotOwnerID;

	[ObservableProperty] private ushort traits1;

	[ObservableProperty] private ushort traits2;

	[ObservableProperty] private ushort turnOns1;

	[ObservableProperty] private ushort turnOns2;

	[ObservableProperty] private ushort turnOffs1;

	[ObservableProperty] private ushort turnOffs2;

	[ObservableProperty] private ushort species;

	[ObservableProperty] private ushort countdownTimerID;

	[ObservableProperty] private ushort perfumeDuration;

	[ObservableProperty] private ushort dateTimer;

	[ObservableProperty] private ushort dateScore;

	[ObservableProperty] private ushort dateUnlockCount;

	[ObservableProperty] private ushort lovePotionDuration;

	[ObservableProperty] private ushort aspirationScoreLock;

	[ObservableProperty] private ushort dateNeighborID;

	[ObservableProperty] private ushort lotID;

	[ObservableProperty] private ushort salary;

	[ObservableProperty] private ushort oFBFlags;

	[ObservableProperty] private ushort jobAssignment;

	[ObservableProperty] private ushort petTraits;

	[ObservableProperty] private ushort subspecies;

	[ObservableProperty] private ushort daysLeftInVacation;

	[ObservableProperty] private ushort turnOns3;

	[ObservableProperty] private ushort turnOffs3;

	[ObservableProperty] private ushort traits3;

	[ObservableProperty] private SdscHobbies hobbies = new();

	[ObservableProperty] private ushort predestinedHobby;

	[ObservableProperty] private ushort lifetimeAspiration;

	[ObservableProperty] private ushort lifetimeAspirationPoints;

	[ObservableProperty] private ushort lifetimeAspirationPointsSpent;

	[ObservableProperty] private ushort decayHungerModifier;

	[ObservableProperty] private ushort decayComfortModifier;

	[ObservableProperty] private ushort decayBladderModifier;

	[ObservableProperty] private ushort decayEnergyModifier;

	[ObservableProperty] private ushort decayHygieneModifier;

	[ObservableProperty] private ushort decayFunModifier;

	[ObservableProperty] private ushort decaySocialModifier;

	[ObservableProperty] private uint bugsCollection;

	[ObservableProperty] private ushort reputation;

	[ObservableProperty] private ushort probabilityToAppear;

	[ObservableProperty] private ushort titlePostName;

	[ObservableProperty] private ushort simInstance;

	[ObservableProperty] private uint simGUID;

	[ObservableProperty] private uint unknown_02;

	[ObservableProperty] private ObservableCollection<ushort> relations = [];

	[ObservableProperty] private byte[] unknown_03;

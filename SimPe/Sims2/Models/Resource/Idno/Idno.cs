...
using SimPe.Sims2.Views.Resource.Idno;

namespace SimPe.Sims2.Models.Resource.Idno;

public partial class Idno(Resource resource) : ObservableObject, IWrapper
{
	...
	[ObservableProperty] private uint uid;

	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodType> type;

	[ObservableProperty] private string subhoodName;

	[ObservableProperty] private uint reserved_00;

	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodEP> requiredEP;

	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodEP> affiliatedEP;

	[ObservableProperty] private uint idFlags;

	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodSeason> firstSeason;


	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodSeason> secondSeason;


	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodSeason> thirdSeason;


	[ObservableProperty] private EnumDisplayNameItem<NeighborhoodSeason> forthSeason;
	...
	public static IWrapper Unserialize(BinaryReader reader, Resource file)
	{
		...
		idno.Panel = new IdnoPanel() { DataContext = idno };
		return idno;
	}
}

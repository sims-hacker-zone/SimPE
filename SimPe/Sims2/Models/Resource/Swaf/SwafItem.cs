...
public partial class SwafItem(Swaf parent, EnumDisplayNameItem<SwafItemType> type) : ObservableObject
{
	...
	[ObservableProperty] private EnumDisplayNameItem<SwafItemType> type = type;

	[ObservableProperty] private EnumDisplayNameItem<SwafItemVersion> version;

	[ObservableProperty] private ushort simInstance;

	[ObservableProperty] private uint guid;

	[ObservableProperty] private EnumDisplayNameItem<WantType> wantType;

	[ObservableProperty] private uint value;

	[ObservableProperty] private ushort property;

	[ObservableProperty] private uint index;

	[ObservableProperty] private int score;

	[ObservableProperty] private int influence;
	...
	public static SwafItem Unserialize(BinaryReader reader, Swaf parent, EnumDisplayNameItem<SwafItemType> type)
	{
		...
		{
			Models.Resource.Swaf.WantType.Skill => reader.ReadUInt16(),
			Models.Resource.Swaf.WantType.Sim => item.Version.Item >= SwafItemVersion.Version8
				? reader.ReadUInt16()
				: (uint)0,
			_ => item.WantType.Item > Models.Resource.Swaf.WantType.Sim
				? reader.ReadUInt32()
				: 0
		};
	...
	public void Serialize(BinaryWriter writer)
	{
		...
		if (WantType.Item == Models.Resource.Swaf.WantType.Skill
		    || (WantType.Item == Models.Resource.Swaf.WantType.Sim
		        && Version >= SwafItemVersion.Version8))
		{
			writer.Write((ushort)Value);
		}
		else if (WantType.Item > Models.Resource.Swaf.WantType.Sim)
		{
			writer.Write(Value);
		}

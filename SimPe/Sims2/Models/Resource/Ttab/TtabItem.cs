...
{
	[ObservableProperty] private Ttab parent = parent;

	[ObservableProperty] private ushort action;

	[ObservableProperty] private ushort guard;

	[ObservableProperty] private ObservableCollection<uint> counts = [];

	[ObservableProperty] private ushort flags;
	[ObservableProperty] private ushort flags2;
	[ObservableProperty] private uint strIndex;
	[ObservableProperty] private EnumDisplayNameItem<AttenuationCode> attenuationCode;
	[ObservableProperty] private float attenuationValue;
	[ObservableProperty] private uint autonomy;
	[ObservableProperty] private uint joinIndex;
	[ObservableProperty] private ushort uIDisplayType;
	[ObservableProperty] private uint facialAnimation;
	[ObservableProperty] private float memoryIterMult;
	[ObservableProperty] private uint objectType;
	[ObservableProperty] private uint modelTableID;
	[ObservableProperty] private TtabItemMotiveHumanTable humanGroups;
	[ObservableProperty] private TtabItemMotiveAnimalTable animalGroups;
	...
	{
		Resource ttas_file = (Parent.Resource.File as PackageFile).FindFile(FileTypes.TTAs,
			(Parent.Resource as Resource).Group,
			(Parent.Resource as Resource).InstanceHigh, (Parent.Resource as Resource).Instance);

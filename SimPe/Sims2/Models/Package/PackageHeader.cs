...
{
	[ObservableProperty] private byte[] magic = "DBPF"u8.ToArray();

	public string MagicString => Encoding.ASCII.GetString(Magic);

	[ObservableProperty] private uint majorVersion;

	[ObservableProperty] private uint minorVersion;

	public Version Version => new((int)MajorVersion, (int)MinorVersion);

	[ObservableProperty] private byte[] reserved_00 = new byte[12];

	[ObservableProperty] private uint created;

	[ObservableProperty] private uint modified;

	[ObservableProperty] private PackageHeaderIndex index;

	[ObservableProperty] private PackageHeaderHole hole;

	[ObservableProperty] private EnumDisplayNameItem<IndexTypes> indexType;

	/// <summary>
	/// The EP icon to show (for Lots)
	/// </summary>
	[ObservableProperty] private short ePIcon;

	/// <summary>
	/// Whether the EP Icon should be shown
	/// </summary>
	[ObservableProperty] private short showIcon;

	[ObservableProperty] private byte[] reserved_02 = new byte[28];

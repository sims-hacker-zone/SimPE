...
{
	[ObservableProperty] private MainWindowViewModel parent = parent;
	...
	public async void OpenRecent(object filepath)
	{
		...
		if (file != null)
		{
			await Common.Models.FileLoader.OpenFile(file);
		}
	}

	public async void OpenIn(object filepath)
	{
		IReadOnlyList<IStorageFile> filelist = await Parent.Parent.StorageProvider.OpenFilePickerAsync(
			new FilePickerOpenOptions
			{
				...
				SuggestedStartLocation =
					await Parent.Parent.StorageProvider.TryGetFolderFromPathAsync(filepath as string)
			});
		if (filelist.Any())
		{

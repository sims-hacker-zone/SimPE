...
using SimPe.Views.PackedFile.Lotd;

namespace SimPe.Models.PackedFile.Lotd
{
	public partial class Lotd(PackedFile file) : ObservableObject, IWrapper
	{
		[ObservableProperty]
		...
		[ObservableProperty]
		private CheckableFlagEnum<LotRoads> lotRoads;

		[ObservableProperty]
		private EnumDisplayNameItem<LotRotation> lotRotation;

		[ObservableProperty]
		private CheckableFlagEnum<LotFlags> lotFlags;
		...
		[ObservableProperty]
		private CheckableFlagEnum<LotHobbyFlags> lotHobbyFlags;
		...
		public static Lotd Unserialize(BinaryReader reader, PackedFile file)
		{
			Lotd lotd = new(file);
			lotd.FileName = Encoding.UTF8.GetString(reader.ReadBytes(64));

...
using SimPe.Views.PackedFile.Objd;

namespace SimPe.Models.PackedFile.Objd
{
	public partial class Objd(PackedFile file) : ObservableObject, IWrapper
	{
		[ObservableProperty]
		...
		[ObservableProperty]
		private EnumDisplayNameItem<ObjdVersion> version;

		[ObservableProperty]
		private ushort initialStackSize;

		[ObservableProperty]
		private ObjdWallAdjacencyFlags defaultWallAdjacentFlags;

		[ObservableProperty]
		private ushort defaultPlacementFlags;

		[ObservableProperty]
		private ushort defaultWallPlacementFlags;

		[ObservableProperty]
		private ushort defaultAllowedHeightFlags;

		[ObservableProperty]
		private ushort interactionTableID;

		[ObservableProperty]
		private ushort interactionGroup;

		[ObservableProperty]
		private EnumDisplayNameItem<ObjdType> type;

		[ObservableProperty]
		private ushort multiTileMasterID;

		[ObservableProperty]
		private ushort multiTileSubIndex;

		[ObservableProperty]
		private ushort useDefaultPlacementFlags;

		[ObservableProperty]
		private ushort lookAtScore;
		...
		[ObservableProperty]
		private ushort bodyStringsID;

		[ObservableProperty]
		private ushort slotsID;

		[ObservableProperty]
		private uint diagonalSelectorGUID;

		[ObservableProperty]
		private uint gridAlignedSelectorGUID;

		[ObservableProperty]
		private ushort objectOwnershipFlags;

		[ObservableProperty]
		private ushort ignoreGlobalSimFieldInCASLot;

		[ObservableProperty]
		private ushort cannotMoveOutWith;

		[ObservableProperty]
		private ushort hauntable;

		[ObservableProperty]
		private uint proxyGUID;

		[ObservableProperty]
		private ushort slotGroup;

		[ObservableProperty]
		private ushort aspirationFlags;

		[ObservableProperty]
		private ushort memoryNiceBad;

		[ObservableProperty]
		private ushort salePrice;

		[ObservableProperty]
		private ushort initialDepreciation;

		[ObservableProperty]
		private ushort dailyDepreciation;

		[ObservableProperty]
		private bool selfDepreciating;
		...
		[ObservableProperty]
		private ushort isGlobalSimObject;

		[ObservableProperty]
		private ushort toolTipNameType;

		[ObservableProperty]
		private ushort templateVersion;

		[ObservableProperty]
		private ushort nicenessMultiplier;

		[ObservableProperty]
		private ushort noDuplicateOnPlacement;

		[ObservableProperty]
		private ushort wantCategory;

		[ObservableProperty]
		private ushort noNewNameFromTemplate;

		[ObservableProperty]
		private ushort objectVersion;

		[ObservableProperty]
		private ushort defaultThumbnailID;

		[ObservableProperty]
		private ushort motiveEffectsID;

		[ObservableProperty]
		private uint jobObjectGUID;

		[ObservableProperty]
		private ushort catalogPopupID;

		[ObservableProperty]
		private ushort ignoreCurrentModelIndexInIcons;

		[ObservableProperty]
		private ushort levelOffset;

		[ObservableProperty]
		private ushort shadowType;

		[ObservableProperty]
		private ushort numAttributes;

		[ObservableProperty]
		private ushort numberOfObjectArrays;

		[ObservableProperty]
		private ushort frontDirection;

		[ObservableProperty]
		private ushort multiTileLeadObject;

		[ObservableProperty]
		private ObjdRequiredEPFlags requiredEPFlags;

		[ObservableProperty]
		private ushort chairEntryFlags;
		...
		[ObservableProperty]
		private uint objectModelGUID;

		[ObservableProperty]
		private ushort footprintMask;

		[ObservableProperty]
		private ushort extendFootprint;

		[ObservableProperty]
		private EnumDisplayNameItem<ObjdObjectSize> objectSize;

		[ObservableProperty]
		private ushort ratingHunger;

		[ObservableProperty]
		private ushort ratingComfort;

		[ObservableProperty]
		private ushort ratingHygiene;

		[ObservableProperty]
		private ushort ratingBladder;

		[ObservableProperty]
		private ushort ratingEnergy;

		[ObservableProperty]
		private ushort ratingFun;

		[ObservableProperty]
		private ushort ratingRoom;

		[ObservableProperty]
		private ObjdRatingSkillFlags ratingSkillFlags;

		[ObservableProperty]
		private ushort numTypeAttributes;

		[ObservableProperty]
		private ushort miscFlags;

		[ObservableProperty]
		private ushort functionSubSort;

		[ObservableProperty]
		private ushort downtownSort;

		[ObservableProperty]
		private ushort keepBuying;

		[ObservableProperty]
		private ushort vacationSort;

		[ObservableProperty]
		private ushort resetLotAction;

		[ObservableProperty]
		private ushort threeDObjectType;

		[ObservableProperty]
		private ushort communitySort;

		[ObservableProperty]
		private ushort dreamFlags;

		[ObservableProperty]
		private ushort thumbnailFlags;

		[ObservableProperty]
		private ushort ratingScratch;

		[ObservableProperty]
		private ushort ratingChew;

		[ObservableProperty]
		private ushort requirements;
		...
		public static IWrapper Unserialize(BinaryReader reader, PackedFile file)
		{
			...
			objd.FileName = Encoding.UTF8.GetString(reader.ReadBytes(nameLength));

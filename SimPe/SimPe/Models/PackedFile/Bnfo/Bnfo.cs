...
using SimPe.Views.PackedFile.Bnfo;

namespace SimPe.Models.PackedFile.Bnfo
{
	public partial class Bnfo(PackedFile file) : ObservableObject, IWrapper
	{
		[ObservableProperty]
		private PackedFile file = file;

		[ObservableProperty]
		private EnumDisplayNameItem<BnfoVersion> version;

		[ObservableProperty]
		private uint currentBusinessState;

		[ObservableProperty]
		private uint highestBusinessState;

		[ObservableProperty]
		private uint unknown_00;

		[ObservableProperty]
		private float unknown_01;

		[ObservableProperty]
		private uint unknown_02;

		[ObservableProperty]
		private ObservableCollection<BnfoCustomer> customers = [];

		[ObservableProperty]
		private ObservableCollection<BnfoEmployee> employees = [];

		[ObservableProperty]
		private ObservableCollection<BnfoHistory> histories = [];
		...
		public static Bnfo Unserialize(BinaryReader reader, PackedFile file)
		{
			Bnfo bnfo = new(file)
			{

...
namespace SimPe.Models.PackedFile.Bhav
{
	...
	{
		[ObservableProperty]
		private Bhav parent = parent;

		[ObservableProperty]
		private ushort opcode;

		[ObservableProperty]
		private ushort goToTrue;

		[ObservableProperty]
		private ushort goToFalse;

		[ObservableProperty]
		private byte nodeVersion;

		[ObservableProperty]
		private byte[] operands = new byte[16];
		...
		public static BhavInstruction Unserialize(BinaryReader reader, Bhav parent)
		{
			...
			instruction.Operands = parent.Version >= 0x8003 ? reader.ReadBytes(16) : [.. reader.ReadBytes(8), 0, 0, 0, 0, 0, 0, 0, 0];
			return instruction;
		}

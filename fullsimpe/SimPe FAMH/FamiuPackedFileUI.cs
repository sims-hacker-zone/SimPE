...
namespace SimPe.Plugin
{
    ...
    public partial class FamiuPackedFileUI : SimPe.Windows.Forms.WrapperBaseControl, IPackedFileUI
    {
            ...
            if (goodsections > 1)
            {
                ...
                for (int i = 0; i < sections; i++)
                {
                    if (TestIsValid(i))
                    {
                        ...
                        cdatas[n] = Convert.ToInt32((filedata[(i * 42) + 34] << 16) + filedata[(i * 42) + 33]);
                        n++;
                        if (n > goodsections) break; // catch Index was outside the bounds of the array Exception
                    }
                }
            }
            else
        ...
        private void filimuptext()
        {
            ...
            if (shwraw)
            {
                if (TestIsValid(currentsectionindex))
                    gtname.Text = "~ Valid Data Block ~ Number " + Convert.ToString(currentsection) + "\r\n";
                else
                    gtname.Text = "~ Invalid Data Block ~ Number " + Convert.ToString(currentsection) + "\r\n";
                /* Only Unknown values
                ...
                */
                ...
                {
                    gtname.Text += "(0x" + Helper.HexString(filedata[(currentsectionindex * 42) + i]) + ")  -  ";
                    gtname.Text += "(0x" + Helper.HexString(filedata[(currentsectionindex * 42) + i + 1]) + ")  -  ";
                    gtname.Text += "(0x" + Helper.HexString(filedata[(currentsectionindex * 42) + i + 2]) + ")\r\n";
                }
            }
            else
            {
                ...
                {
                    LotDescription = SimPe.FileTable.ProviderRegistry.LotProvider.FindLot(filedata[currentsectionindex * 42]);
                    int FamFund = (filedata[(currentsectionindex * 42) + 34] << 16) + filedata[(currentsectionindex * 42) + 33];
                    ...
                    gtname.Text += " " + Wrapper.Subhood(filedata[currentsectionindex * 42]) + "\r\n\r\n";
                    gtname.Text += "Family members present = " + Convert.ToString(filedata[(currentsectionindex * 42) + 1]) + "\r\n"; // byte pair 2
                    gtname.Text += " Men Present = " + Convert.ToString(filedata[(currentsectionindex * 42) + 2]) + "\r\n"; // byte pair 3
                    gtname.Text += " Women Present = " + Convert.ToString(filedata[(currentsectionindex * 42) + 3]) + "\r\n"; // byte pair 4
                    gtname.Text += " Boys Present = " + Convert.ToString(filedata[(currentsectionindex * 42) + 4]) + "\r\n"; // byte pair 5
                    gtname.Text += " Girls Present = " + Convert.ToString(filedata[(currentsectionindex * 42) + 5]) + "\r\n\r\n"; // byte pair 6
                    if (Wrapper.Version == 86) gtname.Text += "Resources = " + Convert.ToString(FamFund) + "\r\n";
                    else gtname.Text += "Family Funds = " + FamFund.ToString("C0") + "\r\n";
                    gtname.Text += "Family Friends = " + Convert.ToString(filedata[(currentsectionindex * 42) + 35]) + "\r\n";
                    /*
        ...
        {
            if (filedata[currentsectionindex * 42] == 0 && !Helper.WindowsRegistry.AllowLotZero) return false; // Lot Number, only sims in a playable family could age a day
            if (filedata[(currentsectionindex * 42) + 1] > 32) return false; // too many sims to be correct
            if (filedata[(currentsectionindex * 42) + 2] + filedata[(currentsectionindex * 42) + 3] + filedata[(currentsectionindex * 42) + 4] + filedata[(currentsectionindex * 42) + 5] != filedata[(currentsectionindex * 42) + 1]) return false; // bad checksum
            return true;
        }
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            ...
            int n = 0;
            int currentsectionindex = currentsection - 1;
            if (filedata[(currentsectionindex * 42)] > 0) goodsections--;
            sections--;
            Array.Resize<ushort>(ref filedata, sections * 42);
            foreach (ushort k in Wrapper.FVal)
            {
                if (l < currentsectionindex * 42 || l + 1 > (currentsectionindex + 1) * 42)
                {
                    filedata[n] = k;
                    n++;
                }
        ...
        private void btediter_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                int currentsectionindex = currentsection - 1;
                int FamFund = (filedata[(currentsectionindex * 42) + 34] << 16) + filedata[(currentsectionindex * 42) + 33];
                tbLotNo.Text = "0x" + Helper.HexString(filedata[(currentsectionindex * 42)]);
                tbMenNo.Text = Convert.ToString(filedata[(currentsectionindex * 42) + 2]);
                tbLadyNo.Text = Convert.ToString(filedata[(currentsectionindex * 42) + 3]);
                tbBoyNo.Text = Convert.ToString(filedata[(currentsectionindex * 42) + 4]);
                tbGirlNo.Text = Convert.ToString(filedata[(currentsectionindex * 42) + 5]);
                tbFunds.Text = Convert.ToString(FamFund);
                tbFriends.Text = Convert.ToString(filedata[(currentsectionindex * 42) + 35]);
                tbEditer.HeaderText = "History Block Editer - Block " + Convert.ToString(currentsection);
        ...
        private void BtGoody_Click(object sender, EventArgs e)
        {
            int currentsectionindex = currentsection - 1;
            bool wasgood = false;
            if (filedata[(currentsectionindex * 42)] > 0) wasgood = true;
            try
            {
                int FamFund = Convert.ToInt32(tbFunds.Text);
                string monee = Helper.HexString(FamFund);
                filedata[(currentsectionindex * 42) + 34] = Convert.ToUInt16(monee.Substring(0, 4), 16);
                filedata[(currentsectionindex * 42) + 33] = Convert.ToUInt16(monee.Substring(4, 4), 16);
                filedata[(currentsectionindex * 42) + 2] = Convert.ToUInt16(tbMenNo.Text);
                filedata[(currentsectionindex * 42) + 3] = Convert.ToUInt16(tbLadyNo.Text);
                filedata[(currentsectionindex * 42) + 4] = Convert.ToUInt16(tbBoyNo.Text);
                filedata[(currentsectionindex * 42) + 5] = Convert.ToUInt16(tbGirlNo.Text);
                filedata[(currentsectionindex * 42) + 1] = (ushort)(filedata[(currentsectionindex * 42) + 2] + filedata[(currentsectionindex * 42) + 3] + filedata[(currentsectionindex * 42) + 4] + filedata[(currentsectionindex * 42) + 5]);
                filedata[(currentsectionindex * 42)] = Convert.ToUInt16(tbLotNo.Text, 16);
                filedata[(currentsectionindex * 42) + 35] = Convert.ToUInt16(tbFriends.Text);
                if (filedata[(currentsectionindex * 42)] > 0 && !wasgood) goodsections++;
                else if (filedata[(currentsectionindex * 42)] == 0 && wasgood) goodsections--;
                this.CanCommit = true;
                RefreshGraphs();
                filimuptext();
            }

...
namespace SimPe
{
	...
	public class About : SimPe.Windows.Forms.HelpForm
    {
		...
		void LoadResource(string flname)
		{
			...
			System.IO.Stream s = this.GetType().Assembly.GetManifestResourceStream("SimPe.docs."+flname+"-"+System.Globalization.CultureInfo.CurrentCulture.TwoLetterISOLanguageName+".rtf");
			if (s==null) s = this.GetType().Assembly.GetManifestResourceStream("SimPe.docs."+flname+"-en.rtf");
			if (s!=null) 
			{
        ...
        void LoadHtmResource(string flname)
        {
            rtb.Visible = false;
            wb.Visible = true;
            System.IO.Stream s = this.GetType().Assembly.GetManifestResourceStream("SimPe.docs." + flname + ".htm");
            if (s != null)
            {
                wb.DocumentStream = s;
            }

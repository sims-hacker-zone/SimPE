...
namespace SimPe.PackedFiles.Ngbh
{
	...
	public class NgbhItemsListView : UserControl
	{
		...
		[Browsable(false)]
		public NgbhItems SelectedNgbhItems
		{
			get
			{
				...
				NgbhItems ret = new NgbhItems(parent);
		...
		)
		{
			...
			NgbhItems nitems = SelectedNgbhItems;
			items.Remove(nitems);
		...
		void CopyItems()
		{
			NgbhItems selitems = SelectedNgbhItems;
			if (selitems.Count > 0)
			{
		...
		public void SelectMemoriesByGuid(NgbhItems items)
		{
			if (items.Length > 0)
			{
				lv.Enabled = false;
		...
		void DeleteItems(bool cascade)
		{
			if (lv.SelectedItems.Count != 0)
			{
				...
				try
				{
					...
					NgbhItems memoryItems = SelectedNgbhItems;

...
namespace SimPe.PackedFiles.UserInterface
{
    class SimOriGuid
    {
        ...
        internal static System.Drawing.Image LoadTurnOnsIcon(int i) // i can be zero
        {
            uint j;
            int to1 = 13;
            if (Helper.WindowsRegistry.LoadOnlySimsStory > 0) to1 = 12; // All Sims Story Editions Don't use Tattoos
        ...
        public static string AboutSim(Wrapper.ExtSDesc Sim)
        {
            ...
            if ((int)Sim.Version >= (int)SimPe.PackedFiles.Wrapper.SDescVersions.Nightlife)
            {
            ...
            {
                if (Sim.CharacterDescription.BodyFlag.Fit) ret += " athletic female";
                else if (Sim.CharacterDescription.BodyFlag.Fat) ret += " overweight female";
                else ret += " female";
            }
            else
            {
                if (Sim.CharacterDescription.BodyFlag.Fit) ret += " athletic male";
                else if (Sim.CharacterDescription.BodyFlag.Fat) ret += " overweight male";
                else ret += " male";
            }

...
namespace SimPe.PackedFiles.Sdsc
{
	class SimOriGuid
	{
		...
		public static void FixOrigGUID(ExtSDesc Sim)
		{
			...
			if (curspec != species)
			{
				Sim.Nightlife.Species =
					(SdscNightlife.SpeciesType)curspec;
				pkg.Close();
			...
			if (pfds.Length == 1)
			{
				...
				else
				{
					...
					{
						Sim.Nightlife.Species =
							(SdscNightlife.SpeciesType)species;
					}
					else
					{
						semig = "PetGlobals";
		...
		public static string AboutSim(ExtSDesc Sim)
		{
			string ret = "";

...
namespace SimPe.PackedFiles.Sdsc
{
	...
	/// </summary>
	public partial class ExtSDescUI
		:
			...
			IPackedFileUI
	{
		...
		public ExtSDescUI()
		{
			strresources = new System.Resources.ResourceManager(typeof(ExtSDescUI));
		...
		void Initialize()
		{
			...
			System.Resources.ResourceManager resources =
				new System.Resources.ResourceManager(typeof(ExtSDescUI));
			Commited += new EventHandler(ExtSDesc_Commited);

			srcRel = new Srel.CommonSrel();
			dstRel = new Srel.CommonSrel();
			//
			// srcRel
			//
		...
		void InitDropDowns()
		{
			...
			foreach (KeyValuePair<uint, string> a in SDesc.AddonCarrers)
			{
			...
			cbmajor.Items.Clear();
			foreach (KeyValuePair<uint, string> a in SDesc.AddonMajors)
			{
			...
			foreach (KeyValuePair<uint, string> a in SDesc.AddonSchools)
			{
			...
			cbEp3Asgn.Enum = typeof(JobAssignment);

			cbSpecies.ResourceManager = Localization.Manager;
			cbSpecies.Enum =
				typeof(SdscNightlife.SpeciesType);

			cbHobbyPre.ResourceManager = Localization.Manager;
			cbHobbyPre.Enum = typeof(Hobbies);

			for (int i = 0; i < cbHobbyEnth.Items.Count; i++)
			{
				Hobbies hb =
					SdscFreetime.IndexToHobbies((ushort)i);
				Type type = typeof(Hobbies);
		...
		public Sdsc.ExtSDesc Sdesc => (Sdsc.ExtSDesc)Wrapper;
		...
		protected override void RefreshGUI()
		{
			...
			try
			{
				...
				biEP2.Enabled =
					(int)Sdesc.Version
						>= (int)SDescVersions.Nightlife
					&& Sdesc.Nightlife.Species == 0
				;
				biEP3.Enabled =
					(int)Sdesc.Version
						>= (int)SDescVersions.Business
					&& Sdesc.Nightlife.Species == 0
					&& (int)Sdesc.Version
						!= (int)SDescVersions.Castaway
				;
				biEP6.Enabled =
					(int)Sdesc.Version
						>= (int)SDescVersions.Voyage
					&& Sdesc.Nightlife.Species == 0
					&& PathProvider.Global.EPInstalled > 9
				;
				biEP7.Enabled =
					(int)Sdesc.Version
						>= (int)SDescVersions.Freetime
					&& Sdesc.Nightlife.Species == 0
				;
				cbSpecies.Enabled =
					(int)Sdesc.Version
					>= (int)SDescVersions.Pets
				;
		...
		void RefreshEP1(Sdsc.ExtSDesc sdesc)
		{
		...
		void RefreshSkills(Sdsc.ExtSDesc sdesc) // Updated Dog skills only for Pet Story
		{
			// should not be reading Nightlife.Species if version is below Pets !!
			if ((int)Sdesc.Version >= (int)SDescVersions.Pets)
			{
				if (
					(
		...
		void RefreshMisc(Sdsc.ExtSDesc sdesc)
		{
		...
		void RefreshId(Sdsc.ExtSDesc sdesc)
		{
		...
		void RefreshCareer(Sdsc.ExtSDesc sdesc)
		{
			...
			cbaspiration.Enabled = (int)sdesc.Version
					< (int)SDescVersions.Freetime || sdesc.Freetime.SecondaryAspiration == AspirationTypes.Nothing;
		}

		void RefreshInterests(Sdsc.ExtSDesc sdesc)
		{
		...
		void RefreshCharcter(Sdsc.ExtSDesc sdesc)
		{
		...
		private void Activate_biLezby(object sender, EventArgs e)
		{
			...
			Sdesc.Freetime.HobbyPredistined = Hobbies.Nature;
...
if (System.IO.File.Exists(Sdesc.CharacterFileName))
{
	...
	if (pfds.Length>0)
	{
		...
		{
			return Sdsc.ExtSDesc.FindRelation(Sdesc, src, dst);
		}
		...
		private void SelectedSimRelationChanged(object sender, EventArgs e)
		{
			...
			Sdsc.ExtSDesc sdesc = (Sdsc.ExtSDesc)
		...
		void RefreshEP2(Sdsc.ExtSDesc sdesc)
		{
			cbSpecies.SelectedValue = sdesc.Nightlife.Species;
		...
		void RefreshEP6(Sdsc.ExtSDesc sdesc)
		{
			ShowAllCollectibles();
		...
		#region EP3
		void RefreshEP3(Sdsc.ExtSDesc sdesc)
		{
		...
		private void ChangedEP3(object sender, EventArgs e)
		{
			...
			try
			{
				...
				Sdesc.Business.Assignment = (JobAssignment)
		...
		#region EP4
		void RefreshEP4(Sdsc.ExtSDesc sdesc)
		{
		...
		{
			SdscNightlife.SpeciesType sp =
				(SdscNightlife.SpeciesType)
					cbSpecies.SelectedValue;
			bool showsim =
				sp == SdscNightlife.SpeciesType.Human;
			return showsim;
		}

		private void SetCharacterAttributesVisibility()
		{
			bool showsim = Sdesc == null || (int)Sdesc.Version < (int)SDescVersions.Pets || Sdesc.Nightlife.IsHuman;
			pnSimInt.Visible = pnHumanChar.Visible = showsim;
			btProfile.Visible = showsim && !Helper.WindowsRegistry.Config.HiddenMode;
			pnPetChar.Visible = pnPetInt.Visible = !showsim;
		}
		...
		#region Freetime
		void RefreshEP7(Sdsc.ExtSDesc sdesc)
		{
			InternalChange = true;

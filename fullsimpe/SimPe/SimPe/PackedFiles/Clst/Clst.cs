...
namespace SimPe.PackedFiles.Clst
{
		...
		: AbstractWrapper, IFileWrapper, IFileWrapperSaveExtension
	{
		...
		internal Clst()
			: base()
		{
			IndexType = IndexTypes.ptShortFileIndex;
		...
		public Clst(IPackedFileDescriptor pfd, IPackageFile package)
			: base()
		{
		...
		{
			IndexType = package.Header.IndexType;
			long count = IndexType == IndexTypes.ptLongFileIndex ? reader.BaseStream.Length / 0x14 : reader.BaseStream.Length / 0x10;
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (i == 2 && !switch_t)
				{
					...
					{
						i = 0;
						IndexType = IndexType == IndexTypes.ptLongFileIndex
							? IndexTypes.ptShortFileIndex
							: IndexTypes.ptLongFileIndex;

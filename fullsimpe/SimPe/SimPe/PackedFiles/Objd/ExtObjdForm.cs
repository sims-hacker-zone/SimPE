...
namespace SimPe.PackedFiles.Objd
{
	...
	internal class ExtObjdForm : Form, IPackedFileUI
	{
		...
		public ExtObjdForm()
		{
			...
			cbtype.Items.Add(ObjectTypes.Unknown);
			cbtype.Items.Add(ObjectTypes.ArchitecturalSupport);
			cbtype.Items.Add(ObjectTypes.Door);
			cbtype.Items.Add(ObjectTypes.Memory);
			cbtype.Items.Add(ObjectTypes.ModularStairs);
			cbtype.Items.Add(ObjectTypes.ModularStairsPortal);
			cbtype.Items.Add(ObjectTypes.Normal);
			cbtype.Items.Add(ObjectTypes.Outfit);
			cbtype.Items.Add(ObjectTypes.Person);
			cbtype.Items.Add(ObjectTypes.SimType);
			cbtype.Items.Add(ObjectTypes.Stairs);
			cbtype.Items.Add(ObjectTypes.Template);
			cbtype.Items.Add(ObjectTypes.Vehicle);
			cbtype.Items.Add(ObjectTypes.Window);
			cbtype.Items.Add(ObjectTypes.UnlinkedSim);
			cbtype.Items.Add(ObjectTypes.Tiles);

			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(BuildFunctionSubSort.none)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Columns
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Stairs
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Pool
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_TallColumns
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Arch
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Driveway
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Elevator
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.General_Architectural
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Garden_Trees
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Garden_Shrubs
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Garden_Flowers
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Garden_Objects
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_Door
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_TallWindow
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_Window
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_Gate
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_Arch
				)
			);
			cbBuildSort.Items.Add(
				new LocalizedBuildSubSort(
					BuildFunctionSubSort.Openings_TallDoor
				)
			);
			if (Helper.WindowsRegistry.Config.HiddenMode)
			{
				cbBuildSort.Items.Add(
					new LocalizedBuildSubSort(
						BuildFunctionSubSort.unknown
					)
				);
			}

			cbsort.Enum = typeof(ObjFunctionSubSort);
			...
			try
			{
				...
				for (int i = 0; i < wrapper.Data.Length; i++)
				{
					...
					try
					{
						...
						{
							object o = ht[name];
							wrapper.Data[i] = o is FlagBase ? (short)(FlagBase)ht[name] : System.Convert.ToInt16(ht[name]);
						}
					}
					catch (Exception ex)
					{
		...
		private static void readGLUAObjDef()
		{
			...
			List<ObjLuaConstant> loc = new List<ObjLuaConstant>(
				(ObjLuaConstant[])new ObjLua().ProcessFile(objDefPFD, glua).Root.Constants.ToArray(typeof(ObjLuaConstant))
			);
			...
			while (loc.Count > 0)
			{
				...
				loc.RemoveAt(0);
				int key = System.Convert.ToInt32(loc[0].Value);
				loc.RemoveAt(0);
				if (started)
				{
					names[key] = value;
		...
		private static void readPJSEGlobalStringObjDef()
		{
			...
			Str.Str objDef = new Str.Str().ProcessFile(objDefPFD, gs);
		...
		public void UpdateGUI(IFileWrapper wrapper)
		{
			...
			try
			{
				...
				{
					ObjectTypes ot = (ObjectTypes)cbtype.Items[i];
					if (ot == objd.Type)
					{
				...
				tbPrice.Text = "�" + System.Convert.ToString(objd.Price);
				...
				if (objd.BuildType.Value != 0)
				{
					...
					for (int i = 0; i < cbBuildSort.Items.Count; i++)
					{
						...
						BuildFunctionSubSort at = o.GetType() == typeof(Alias)
							? (BuildFunctionSubSort)(LocalizedBuildSubSort)
								((Alias)o).Id
							: (BuildFunctionSubSort)(LocalizedBuildSubSort)o;
				...
				llgetGUID.Visible =
					...
					&& objd.Type != ObjectTypes.UnlinkedSim
				;
		...
		private void ChangeType(object sender, EventArgs e)
		{
			...
			try
			{
				...
				llgetGUID.Visible =
					UserVerification.HaveUserId
					&& wrapper.Type != ObjectTypes.Person
					&& wrapper.Type != ObjectTypes.UnlinkedSim
				;
		...
		private void SetGuide(object sender, EventArgs e)
		{
			...
			try
			{
				wrapper.Guid = System.Convert.ToUInt32(tbguid.Text, 16);
				wrapper.Changed = true;
			}
			catch (Exception) { }
			finally
		...
		private void SetGuid(object sender, EventArgs e)
		{
			...
			try
			{
				wrapper.ProxyGuid = System.Convert.ToUInt32(tbproxguid.Text, 16);
				wrapper.OriginalGuid = System.Convert.ToUInt32(tborgguid.Text, 16);
				wrapper.DiagonalGuid = System.Convert.ToUInt32(tbdiag.Text, 16);
				wrapper.GridAlignedGuid = System.Convert.ToUInt32(tbgrid.Text, 16);
				wrapper.Changed = true;
				if (!Helper.WindowsRegistry.Config.HiddenMode && UserVerification.HaveUserId)
				{
		...
		private void cbsort_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Tag = true;
			wrapper.FunctionSubSort = (ObjFunctionSubSort)cbsort.SelectedValue;
			wrapper.Changed = true;
		...
		private void cbBuildSort_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			if (cbBuildSort.SelectedIndex == 0) // none
			{
				...
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 1)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Columns;
				wrapper.Type = ObjectTypes.ArchitecturalSupport;
			}
			if (cbBuildSort.SelectedIndex == 2)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Stairs;
				wrapper.Type = ObjectTypes.Stairs;
			}
			if (cbBuildSort.SelectedIndex == 3)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Pool;
				skippy = true;
			}
			if (cbBuildSort.SelectedIndex == 4)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_TallColumns;
				wrapper.Type = ObjectTypes.ArchitecturalSupport;
			}
			if (cbBuildSort.SelectedIndex == 5)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Arch;
				wrapper.Type = ObjectTypes.ArchitecturalSupport;
			}
			if (cbBuildSort.SelectedIndex == 6)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Driveway;
				skippy = true;
			}
			if (cbBuildSort.SelectedIndex == 7)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Elevator;
				skippy = true;
			}
			if (cbBuildSort.SelectedIndex == 8)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.General_Architectural;
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 9)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Garden_Trees;
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 10)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Garden_Shrubs;
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 11)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Garden_Flowers;
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 12)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Garden_Objects;
				wrapper.Type = ObjectTypes.Normal;
			}
			if (cbBuildSort.SelectedIndex == 13)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_Door;
				wrapper.Type = ObjectTypes.Door;
			}
			if (cbBuildSort.SelectedIndex == 14)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_TallWindow;
				wrapper.Type = ObjectTypes.Window;
			}
			if (cbBuildSort.SelectedIndex == 15)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_Window;
				wrapper.Type = ObjectTypes.Window;
			}
			if (cbBuildSort.SelectedIndex == 16)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_Gate;
				wrapper.Type = ObjectTypes.Door;
			}
			if (cbBuildSort.SelectedIndex == 17)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_Arch;
				wrapper.Type = ObjectTypes.Door;
			}
			if (cbBuildSort.SelectedIndex == 18)
			{
				wrapper.BuildSubSort = BuildFunctionSubSort.Openings_TallDoor;
				wrapper.Type = ObjectTypes.Door;
			}
		...
		private void tbPrice_TextChanged(object sender, EventArgs e)
		{
			...
			try
			{
				...
				wrapper.Price = System.Convert.ToInt16(prise);
			}
			catch
			{

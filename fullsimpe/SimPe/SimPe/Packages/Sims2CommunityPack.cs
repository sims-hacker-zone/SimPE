...
namespace SimPe.Packages
{
	...
	public class Sims2CommunityPack
	{
		...
		/// </summary>
		public const CompressionStrength DEFAULT_COMPRESSION_STRENGTH =
		...
		public static MemoryStream Create(S2CPDescriptor[] files, bool extension)
		{
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				if (extension)
				{
					...
					xml +=
						"<Compressed>"
						+ ((int)files[i].Compressed).ToString()
						+ "</Compressed>"
						+ (char)0x0a;
					...
					case "Sims2CommuniytExt":
					{
						foreach (XmlNode subnode in mainnode)
						{
							switch (subnode.LocalName)
							{
								...
								case "Compressed":
								{
									s2cp.Compressed = (CompressionStrength)
								...
								case "Dependency":
								{
									...
									S2CPDescriptorBase[] deps = new S2CPDescriptorBase[
										al.Count
									];
									al.CopyTo(deps);

...
namespace SimPe.Plugin
{
	...
	public class ImageLoader
	{
		...
		public static DDSData[] ParesDDS(string flname)
		{
			...
			try
			{
				...
				for (int i = 0; i < maps.Length; i++)
				{
					...
					sz = new Size(
						Math.Max(1, sz.Width / 2),
						Math.Max(1, sz.Height / 2)
					);
					firstsize =
						Math.Max(1, sz.Width / 4)
						* Math.Max(1, sz.Height / 4)
						* blocksize;
				}
			}
			finally
		...
		//
		public static Image DXT3Parser(
			Size parentsize,
			TxtrFormats format,
			int imgsize,
			System.IO.BinaryReader reader,
		...
		)
		{
			...
			try
			{
				...
				if (
					(format == TxtrFormats.DXT3Format)
					|| (format == TxtrFormats.DXT5Format) /* || (format == DXT1 RGBA) */
				)
				{
				...
				for (int y = 0; y < bm.Height; y += 4) // DXT encodes 4x4 blocks of pixel
				{
					for (int x = 0; x < bm.Width; x += 4)
					{
						...
						// separate R,G,B
						int color1r = Convert.ToByte(
							((c1packed16 >> 11) & 0x1F)
								* 8.2258064516129032258064516129032
						);
						int color1g = Convert.ToByte(
							((c1packed16 >> 5) & 0x3F)
								* 4.047619047619047619047619047619
						);
						int color1b = Convert.ToByte(
							(c1packed16 & 0x1F) * 8.2258064516129032258064516129032
						);

						int color2r = Convert.ToByte(
							((c2packed16 >> 11) & 0x1F)
								* 8.2258064516129032258064516129032
						);
						int color2g = Convert.ToByte(
							((c2packed16 >> 5) & 0x3F)
								* 4.047619047619047619047619047619
						);
						int color2b = Convert.ToByte(
							(c2packed16 & 0x1F) * 8.2258064516129032258064516129032
						);
		...
		protected static void DXT3WriteTransparencyBlock(
		...
		protected static void DXT5WriteTransparencyBlock(
			System.IO.BinaryWriter writer,
			Color[] alphas
		)
		{
		...
		{
			return Math.Pow(table.R - test.R, 2)
				+ Math.Pow(table.G - test.G, 2)
				+ Math.Pow(table.B - test.B, 2);
		}

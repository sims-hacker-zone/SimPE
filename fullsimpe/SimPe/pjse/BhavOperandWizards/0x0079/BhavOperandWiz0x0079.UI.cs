...
namespace whse.PrimitiveWizards.Wiz0x0079
{
	public partial class UI : UserControl, pjse.iBhavOperandWizForm
	{
		...
		{
			if (!WizardHelpers.Hex32_IsValid(sender))
				return;
		...
		private void OnGuidValidating(object sender, CancelEventArgs e)
		{
			...
			e.Cancel = true;
			((Control)sender).Text = "0x" + SimPe.Helper.HexStringInt(inst.Operands[OperandConstants.Operand0] | inst.Operands[OperandConstants.Operand1] << 8 | inst.Operands[OperandConstants.Operand2] << 16 | inst.Operands[OperandConstants.Operand3] << 24);
